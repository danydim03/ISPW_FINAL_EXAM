...
public class VisualizzaOrdiniController {
    ...
    public List<Ordine> getOrdiniByStato(StatoOrdine stato) throws Exception {
        OrdineDAOInterface dao = DAOFactoryAbstract.getInstance().getOrdineDAO();
        return dao.getOrdiniByStato(stato);
    }

    public void aggiornaStatoOrdine(Long numeroOrdine, StatoOrdine nuovoStato) throws Exception {
        OrdineDAOInterface dao = DAOFactoryAbstract.getInstance().getOrdineDAO();
